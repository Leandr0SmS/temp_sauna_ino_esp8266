 ...
 *
 *  Podemos acessar os dados via pagina http://sauna.local
 *  
 ...
 *  
 *  Podemos supervisionar status do sistema pelo link http://"ip da conexao"  port 80  ou sauna.local
...
// pisca led 
// ESP8266 (NodeMCU LED = gpio16 D0  /  ESP12 led = GPIO02 D4 )
#define led8266 2
//#define led8266 D4
...
//==============================================
void ApagaLed( ) {digitalWrite(led8266, HIGH);}
void LigaLed() {digitalWrite(led8266, LOW);}
...
".btn{background:#3498db;color:#fff;cursor:pointer}</style>";
 ...
 String chamadabotoes = 
  "<form name=comandos>"
  "<input type=submit onclick=resetnet() class=btn value=ResetNet>"
  "<input type=submit onclick=check(this.form) class=btn value=UpdateFirmware></form>"
  "<script>"
  "function check(form) {window.open('/update')}"
  "function resetnet() { window.open('/resetnet') }"
  "</script>" + style;
...
String PageHTML() {
  ...
  txtpage += "body {padding: 1em;} ";
  txtpage += ".title {text-align: center;font-size: 4em;} ";
  txtpage += "#termometro-container {display: flex;justify-content: center;align-items: center;} ";
  txtpage += "#termometro-div {font-size: 2.5em;border-radius: 35%;border: 0.5rem solid #1b6af3;";
  txtpage += "background: radial-gradient(circle at center, #fb623c, #fcfe7b);padding: 1em;color: white;";
  txtpage += "display: flex;justify-content: center;align-items: center;font-weight: bold;height: 17rem;width: 17rem;} ";
  txtpage += "#optionsIcon:hover{cursor: pointer;} ";
  txtpage += "#status{margin-top: 1.5rem;font-size: 0.7em;display: none;} ";
  txtpage += ".space {padding-left: 2rem;} ";
  txtpage += ".center{text-align: center;} ";
  txtpage += ".sub-title{font-weight: 700;} ";
  txtpage += ".hide {opacity: 0;} ";
  txtpage += ".show {opacity: 1;} ";
  txtpage += "#termometro-container, #status {transition: opacity 0.3s;} ";
  txtpage += "</style>";
  txtpage += "</head>";
  ...
  txtpage += "        setTimeout(() => { status.style.display = 'block'; tempContainer.style.display = 'none'; }, 300);";
  txtpage += "      } else if ((status.className === 'show')) {";
  txtpage += "        status.className = 'hide';";
  txtpage += "        tempContainer.className = 'show';";
  txtpage += "        setTimeout(() => { status.style.display = 'none'; tempContainer.style.display = 'flex'; }, 300);";
  txtpage += "      } else {";
  txtpage += "        status.className = 'show';";
  txtpage += "        tempContainer.className = 'show';";
  txtpage += "        setTimeout(() => { status.style.display = 'block'; tempContainer.style.display = 'flex'; }, 300);";
  txtpage += "      }";
  txtpage += "    };";
...
void setup()
{
  ...
  // modulo led ESP8266
  pinMode(led8266, OUTPUT);     // Initialize the LED_BUILTIN pin as an output
  LigaLed();
...
void loop()
{
  ...
  MDNS.update();
  //======================================================= 
  // Mede periodicamente a temperatura da sauna
...
void LerTemperatura() {
  ...
  temp = (temp / 120) * 100;  // reduz 20% do valor para corrigir erro
  lcd.clear();  
  lcd.setCursor(0, 0);               // Set the cursor to the first column and first row
  lcd.print(" TEMP. INTERNA ");     // Print some text
  lcd.setCursor(4,1);
  ...
  //Enviar msg de texto com o valor da temperatura para debug serial
